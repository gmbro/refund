      ...
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
        <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-200 w-full max-w-sm">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-slate-500 mt-1">환불원정대 관리 시스템</p>
          </div>
          <form onSubmit={handleLogin} className="space-y-4">
            <input 
              type="password" 
              placeholder="비밀번호 입력" 
              className="w-full border border-slate-300 rounded-lg px-4 py-3 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition-all"
              value={password}
    ...
    <div className="p-8 max-w-6xl mx-auto font-gowun">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <button onClick={() => fetchRequests()} className="px-4 py-2 bg-slate-100 rounded-lg text-sm hover:bg-slate-200 transition-colors">새로고침</button>
          <button onClick={handleLogout} className="px-4 py-2 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100 font-medium transition-colors">로그아웃</button>
        </div>
      </div>
